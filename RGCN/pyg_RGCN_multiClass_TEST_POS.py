...
########    neg_out = model.decode(z, orig_neg_edge_index, 0)#, orig_neg_edge_type)
    ...
    out = torch.cat([pos_class1_out, pos_class2_out,pos_class3_out, pos_class4_out, pos_class5_out])
    true_labels = torch.cat([torch.ones_like(pos_class1_out), torch.full_like(pos_class2_out, 2), torch.full_like(pos_class3_out, 3), torch.full_like(pos_class4_out, 4), torch.full_like(pos_class5_out, 5)])
...
##FOR DEBUG PURPOSEs
    ...
    for j in range(1,6):
        CLASS_FSCORE[j-1][i-1]=f1_score[j]
