...
# - Up/downsampling: divisor in {5, 10} for majority class, multiplier in {2, 3}
#   for minority classes -> 4 combos: (5,2), (5,3), (10,2), (10,3)
...
# No class 0 means no majority class to downsample anymore.
# Every remaining class (1-5) gets SMOTE-upsampled by this multiplier.
# Two configs tried: upsample x2 and upsample x3.
sampling_values = [2, 3]
...
gt_data = gt_data[~gt_data["INTERACTS"].isin([0, 6, 7, 8])]  # exclude class 0 and unused classes
...
def export_results_to_excel(embed_name, df_inner, df_outer, overall_cm, unique_classes,
    ...
    r += 1
    config = {
        "Best Sampling (upsample multiplier)": str(best_sampling),
    ...
    output_path = f"{embed_name}_results_test_smote_v2.xlsx"
    wb.save(output_path)
...
for embed in EMBEDDINGS_TO_RUN:
    ...
    for fold_idx, (train_index, test_index) in enumerate(outer_cv.split(X_all, y_all)):
        ...
        for j in sampling_values:
            # No class 0, so there is no majority class to downsample anymore.
            # Every remaining class (1-5) is SMOTE-upsampled by multiplier j.
            class_frames = {cls: train_data[train_data['INTERACTS'] == cls] for cls in unique_classes}

            upsampled_targets = {
                c: max(1, int(len(f) * j)) for c, f in class_frames.items()
            }
            ...
            # X_train_upsampled, y_train_upsampled = smote.fit_resample(X_for_smote, y_for_smote)

            # X_train_upsampled = X_train_upsampled.values
            # y_train_upsampled = y_train_upsampled.values

            # Shuffle (SMOTE output is grouped by class)
            # shuffle_idx = np.random.RandomState(42).permutation(len(y_train_upsampled))
            # X_train_upsampled = X_train_upsampled[shuffle_idx]
            # y_train_upsampled = y_train_upsampled[shuffle_idx]

            # Put SMOTE directly inside the pipeline
            ...
            # Pass the RAW, non-upsampled outer train data to GridSearchCV
            ...
            inner_cv.split(X_for_smote, y_for_smote)
                ...
                inner_fold_pipeline.fit(X_inner_train, y_inner_train)   # SMOTE applied only to this fold's train data
        ...
        for cls in unique_classes:
            ...
            outer_records.append({
                'Outer Fold': fold_idx + 1,
                'Sampling Config': f"up={best_sampling_fold}",
