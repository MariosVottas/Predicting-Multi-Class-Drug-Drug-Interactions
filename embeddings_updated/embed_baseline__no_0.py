# =============================================================================
# BASELINE Random Forest with PyKEEN graph embeddings (TransE, HoLE)
# - Plain RandomForestClassifier() with ALL sklearn defaults, no class_weight
# - Simple 10-fold CV only (no inner loop, no GridSearchCV, no resampling)
# - Class 0 INCLUDED (original full label set)
...
gt_data = gt_data[~gt_data["INTERACTS"].isin([0, 6, 7, 8])]  # keep only desired classes (class 0 included)
...
def export_results_to_excel(embed_name, df_outer, overall_cm, unique_classes):
    df_summary_rows = []
    ...
    r = 1
    ws_summary.cell(row=r, column=1, value=f"{embed_name} -- Baseline RF 10-Fold CV Summary (Average per Class)")
    ...
    r += len(df_summary) + 3
    ws_summary.cell(row=r, column=1, value="Model Configuration").font = Font(name="Arial", bold=True, size=12, color="1F4E79")
    r += 1
    config = {
        "Classifier": "RandomForestClassifier() -- all sklearn defaults, no class_weight",
        "CV scheme": f"Simple {OUTER_K}-fold StratifiedKFold (no inner loop, no tuning, no resampling)",
    }
    ...
    ws_outer = wb.create_sheet("Per-Fold Results")
    r = 1
    ws_outer.cell(row=r, column=1, value=f"{embed_name} -- Per-Fold Per-Class Metrics (10 folds)")
    ...
    for col in range(1, len(df_outer.columns) + 1):
        ws_outer.column_dimensions[get_column_letter(col)].width = 16

    # ---- Sheet 3: Confusion Matrix ----
    ws_cm = wb.create_sheet("Confusion Matrix")
    ws_cm.cell(row=1, column=1, value=f"{embed_name} -- Overall Confusion Matrix (summed across all 10 folds)")
    ...
    output_path = f"{embed_name}_baseline_results_no_0.xlsx"
    wb.save(output_path)
    print(f"\n[{embed_name}] Baseline results saved to: {output_path}")
...
for embed in EMBEDDINGS_TO_RUN:
    ...
    # =========================================================================
    # Baseline: simple 10-fold CV, plain RandomForestClassifier(), no tuning,
    # no resampling. No "inner folds" exist in this design, so the per-fold
    # records below are simply the 10 outer-fold results.
    # =========================================================================
    ...
    for fold_idx, (train_index, test_index) in enumerate(outer_cv.split(X_all, y_all)):
        ...
        fold_report = classification_report(
            y_test, y_pred, labels=unique_classes, output_dict=True, zero_division=0
        )
        ...
        for cls in unique_classes:
            cls_key = str(cls)
            metrics = fold_report.get(cls_key, {})
